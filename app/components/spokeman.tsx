    ...
    return (
        <div className="w-full sm:p-20 px-2 py-10">
            <div className="max-w-6xl mx-auto">
                <div className={`${Avilock.className} uppercase sm:text-7xl text-6xl  font-semibold text-center sm:mb-15 my-5`}>diễn giả workshop</div>
                    ...
                    {lists.map(p => {
                        return (
                            <div key={p.id} className="flex flex-col sm:flex-row items-center gap-4 p-4 py-6 border border-[#132478] rounded-2xl">
                                ...
                                <div className="sm:w-5/6 font-light text-xs flex flex-col gap-1 text-center sm:text-left">
                                    <div className={`font-semibold uppercase text-4xl ${Avilock.className}`}>{p.name}</div>
